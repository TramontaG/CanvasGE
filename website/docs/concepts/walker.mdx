...
Under the hood, `Walker.tick()` updates `gameObject.speed` every tick, and the scene physics step integrates that velocity afterward.
...
- Tune `speed` together with your scene scale and tick rate until the pathing feels right for your game.
